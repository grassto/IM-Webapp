 ...
 * 这些信息最后可以由排序来组织到
 */
Ext.define('IM.utils.SocketEventUtil', {
    ...
    createOwnWelcomeMsg(creatorID, memIDs) {
        ...
        for (var i = 0; i < memIDs.length; i++) {
            if (User.ownerID != memIDs[i]) {
                memName = AddDataUtil.getName(memIDs[i]);
    ...
    createOtherWelcomeMsg(creatorID, memIDs) {
        var creatorName = AddDataUtil.getName(creatorID),
            result = '',
            memNames = '',
            memName = '';
    ...
    createMeRemoveSBMsg(removerID, userID) {
        var userName = AddDataUtil.getName(userID);
        return '你将' + userName + '移出了群聊';
    },
    createSBRemoveMeMsg(removerID, userID) {
        var removerName = AddDataUtil.getName(removerID),
            userName = AddDataUtil.getName(userID);
    ...
    createMeAddSBMsg(operatorID, userIDs) {
        ...
        for (var i = 0; i < userIDs.length; i++) {
            name = AddDataUtil.getName(userIDs[i]);
    ...
    createSBAddSBMsg(operatorID, userIDs) {
        ...
        // 创建者姓名
        name = AddDataUtil.getName(operatorID);
    ...
    addChatMems(chatID, userIDs) {
        ...
        if (store) {
            for (var i = 0; i < userIDs.length; i++) {
                store.add({
                    chat_id: chatID,
                    user_name: AddDataUtil.getName(userIDs[i]),
                    user_id: userIDs[i],
        ...
        for (var i = 0; i < User.allChannels.length; i++) {
            if (User.allChannels[i].chat.chat_id == chatID) {
                for (var j = 0; j < userIDs.length; j++) {
                    ...
                    userName = AddDataUtil.getName(userIDs[j]);
...
/* *********************************** 更换管理员 **********************************************************/
    ...
    // 你将群主转让给xxx
    meChgMgrToSB(newMgr) {
        var name = AddDataUtil.getName(newMgr);
        return '你将群主转让给' + name;
    },

    //  xxx将群主转让给你
    sbChgMgrToMe(oldMgr) {
        var name = AddDataUtil.getName(oldMgr);
        return name + '将群主转让给你';
    },
    ...
    createSBChgHeaderMsg(header, opID) {
        var name = AddDataUtil.getName(opID);
