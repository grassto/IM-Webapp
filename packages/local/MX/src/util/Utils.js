    ...
    getFullUrl2(path) {
        ...
        if (!Ext.isEmpty(path)) {
            ...
            if (Utils.isUrl(path) || /\.json$/i.test(path)) {
                ...
                if (/^(ajax|store)\/[^/]+\/[^/]+$/.test(path)) {
                    const arr = path.split('/');
                    url = Utils.joinPath(Config.httpAIOUrl, 'ApiEntry', `${Ext.String.capitalize(arr[0])}.ashx?class=${arr[1]}&method=${arr[2]}`);
                } else {
                    url = Utils.joinPath(Config.httpAIOUrl, path);
                }
            }
