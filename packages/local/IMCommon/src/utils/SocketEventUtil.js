 ...
 */
Ext.define('IMCommon.utils.SocketEventUtil', {
        ...
        if (data.user_id != User.ownerID) {
            ...
            me.afterHandleRct(me, data, rctView).then((rctRecord) => {
                ...
                if (msgView) {
                    ...
                    me.promptFakeRead(data, rctView.getStore());

                    // 在这个会话，则将服务器端未读去除，防止本地加载重复的消息
                    // ChatUtil.setUnReadToRead(rctView.getStore(), data.chat_id);
                } else {
    ...
    afterHandleRct(me, data, rctView) {
        ...
        return new Ext.Promise((resolve, reject) => {
            ...
            if (rctRecord) {
                ...
                // me.chgRctByPost(rctRecord, data);
            } else {
                me.addRctByWsPost(rctView.getStore(), data); // 添加最近会话数据至页面
                // me.addRctByPost(rctView.getStore(), data);
            }
    ...
    chgRctByPost(rctRecord, data) {
        var me = this;
        data = me.getMsgByType(data);

        var res = me.getChatMembers(data);
        rctRecord.set({
            last_msg_type: data.msg_type,
            last_post_msg: data.content,
            last_post_at: new Date(data.create_at),
        ...
        me.getChatMems(data).then(res => {
            ...
            if (data.chat_type == ChatType.Direct) {
                ...
                for(var i = 0; i < res.members.length; i++) {
                    m.push(res.members[i].user_name);
                }
                name = m.join(',');
                data.header = name;
            }
    ...
    handleGrpAddEvent(msg) {
        var ng = {
            content: JSON.parse(msg.data.user_ids),
            content_ex: JSON.parse(msg.data.user_names),
            operator_id: msg.data.creator_id,
            operator_name: msg.data.creator_name
        };

        var data = {
            chat_id: msg.data.chat_id,
            msg_type: ChatType.Group,
            message: ParseUtil.getCreateNotice(ng),
